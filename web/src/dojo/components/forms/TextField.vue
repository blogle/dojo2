...
<template>
  <label class="field" data-cy="text-field-root">
    <span v-if="label || $slots.label" class="field__label">
      <slot name="label">{{ label }}</slot>
    </span>

    <span class="field__control-shell">
      <span v-if="$slots.prefix" class="field__prefix" aria-hidden="true">
        <slot name="prefix" />
      </span>

      <input
        class="field__control"
        :class="{ 'field__control--with-prefix': !!$slots.prefix }"
        :value="modelValue"
...
.field__control::placeholder {
  color: color-mix(in srgb, var(--color-on-surface-muted) 60%, transparent);
}
