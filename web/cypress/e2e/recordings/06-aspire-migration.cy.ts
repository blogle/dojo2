describe("Aspire migration recording", () => {
  it("imports the deterministic Aspire fixture through Dojo onboarding", () => {
    ...
    cy.contains("label", "Google Sheet ID")
      .find("input")
      .type("fixture")
      .should("have.value", "fixture");
    cy.intercept("POST", "**/import/google-sheet/analyze").as("analyzeFixture");
    cy.contains("button", "Submit").click();
    cy.wait("@analyzeFixture").its("response.statusCode").should("equal", 200);
    ...
    cy.intercept("POST", "**/import/google-sheet/commit").as("commitFixture");
    cy.contains("button", "Continue").click();
    cy.wait("@commitFixture").its("response.statusCode").should("equal", 200);
