...
| Spec area | Requirement | Current status | Frontend support | API support | Persistence/model support | Data available | Blocking gap | Planned work item | Exposure decision |
| --------- | ----------- | -------------- | ---------------- | ----------- | ------------------------- | -------------- | ------------ | ----------------- | ----------------- |
| Assets & Liabilities | Grouped overview and stacked rows | Implemented | Grouped rows include restricted assets and truthful unavailable states | `/api/assets-liabilities` uses type-aware values | Rich value/component tables exist | Yes | Product-owner browser acceptance remains | 6.5 | Expose |
| Assets & Liabilities | Add entity wizard | Implemented | Five entity types, institution suggestions, category configuration, and opening rich values | `POST /api/accounts` creates atomic rich openings | Class detail tables exist | Yes | Product-owner browser acceptance remains | 6.5 | Expose |
...
| Assets & Liabilities | Tracking snapshot detail | Implemented | Snapshot correction, source/date/freshness, history, and trend | Snapshot create/list uses non-future SCD correction | Legacy valuation table stores history | Yes | Product-owner browser acceptance remains | 6.5 | Expose |
| Assets & Liabilities | Tangible asset detail | Implemented | Opening/current valuations and correction history | Valuation create/list feeds all read surfaces | Valuation table exists | Yes | Product-owner browser acceptance remains | 6.5 | Expose |
| Assets & Liabilities | Investment detail and transfers | Implemented | Cash-only/holdings statements, configured categories, contribution/withdrawal previews | Canonical statement value, same-day provisional deltas, derived category activity | Position/cash/price/link tables | Yes | Product-owner browser acceptance remains | 6.5 | Expose |
| Assets & Liabilities | Loan detail and payment attribution | Implemented | Opening principal, configured payment category, actual/derived/estimated/unknown sections | Aggregate reconciliation, YTD checkpoints, projection endpoint | Loan details/snapshots/attributions | Yes | Product-owner browser acceptance remains | 6.5 | Expose |
| Assets & Liabilities | One-to-many tracking cutover | Implemented | Multi-successor modal with variance preview | Atomic idempotent cutover endpoint | Immutable cutover/successor relations with inclusive activation | Yes | Product-owner browser acceptance remains | 6.5 | Expose |
