...
| Spec area | Requirement | Current status | Frontend support | API support | Persistence/model support | Data available | Blocking gap | Planned work item | Exposure decision |
| --------- | ----------- | -------------- | ---------------- | ----------- | ------------------------- | -------------- | ------------ | ----------------- | ----------------- |
| Assets & Liabilities | Grouped overview and stacked rows | Implemented with placeholders | Grouped full-width rows route to details | `/api/assets-liabilities` groups entities | Common account model exists | Partial | Change, dates, attention, and rich values are incomplete | 6.5 | Expose while removing fabricated states |
| Assets & Liabilities | Add entity wizard | Partially implemented | Five entity types and detail forms exist | `POST /api/accounts` supports basic creation | Class detail tables exist | Partial | Confirmation/onboarding link and opening rich values need completion | 6.5 | Expose current wizard |
| Assets & Liabilities | Budget account detail | Substantially implemented | Scoped ledger, filters, summary, trend, configuration | Scoped transactions and summaries exist | Ledger source is authoritative | Yes | Reconciliation/history/notes placeholders | 6.5 / Phase 9 | Expose truthful supported data only |
| Assets & Liabilities | Tracking snapshot detail | Partially implemented | History and trend exist; Add snapshot is inert | Snapshot append/list exists | Legacy valuation table stores history | Yes | Detail uses wrong current value; no correction flow | 6.5 | Expose after value fix |
| Assets & Liabilities | Tangible asset detail | Not implemented | Falls through to generic ledger detail | Valuation append/list exists | Valuation table exists | Yes | Valuations are not read by totals/detail | 6.5 | Keep generic controls hidden until complete |
| Assets & Liabilities | Investment detail and transfers | Not implemented beyond shell | Generic ledger and placeholder metrics | Position/cash/price and transfer endpoints exist | Tables and link behavior exist | Partial | No canonical valuation, reconciliation, link API, or flows | 6.5 | Keep inert actions hidden until complete |
| Assets & Liabilities | Loan detail and payment attribution | Not implemented beyond shell | Generic ledger and placeholder metrics | Loan snapshot append/list exists | Loan details/snapshots exist | Partial | No authoritative obligation, attribution, or statement reconciliation | 6.5 | Keep inert actions hidden until complete |
| Assets & Liabilities | One-to-many tracking cutover | Visual stub only | Modal closes without persistence | No cutover endpoint | No replacement relation/effective lifecycle | No | Atomic final reconciliation and successor creation required | 6.5 | Keep submit disabled until complete |
| Assets & Liabilities | Pending/Cleared settlement states | Implemented for ledger activity | Budget ledger shows pending and cleared | Account and transaction APIs return states | Ledger supports settlement | Yes | Rich statement values use reconciliation state instead | 6.4 | Expose where ledger-backed |
