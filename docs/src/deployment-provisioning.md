...
If the source database is lost, use the break-glass procedure in [Backup and restore](backup-and-restore.md): bootstrap a temporary API/database, authorize Google Drive again, select the existing folder with Picker, and point the restore Job at that temporary API. Do not attempt to recover access through the unavailable source database. The rehearsal must upload through the source API, then restore the same snapshot through only the fresh API.
